...
/**
 * Standard note under service price.
 */
if ( ! function_exists( 'cmbwc_get_default_service_price_note' ) ) {
 ...
 */
if ( ! function_exists( 'cmbwc_get_product_options_for_service_select' ) ) {
...
if ( ! function_exists( 'cmbwc_get_service_options' ) ) {
	function cmbwc_get_service_options() {
		...
		foreach ( $service_options as $key => $option ) {
			if ( ! is_array( $option ) ) {
				unset( $service_options[ $key ] );
				continue;
			}

			$price_type = isset( $option['price_type'] ) && 'per_cover' === $option['price_type'] ? 'per_cover' : 'fixed';
			$is_deposit = isset( $option['is_deposit'] ) && 'yes' === $option['is_deposit'] ? 'yes' : 'no';

			$service_options[ $key ]['label']             = isset( $option['label'] ) ? (string) $option['label'] : (string) $key;
			$service_options[ $key ]['price']             = isset( $option['price'] ) ? (float) $option['price'] : 0;
			$service_options[ $key ]['price_type']        = $price_type;
			$service_options[ $key ]['is_deposit']        = $is_deposit;
			$service_options[ $key ]['price_note']        = ! empty( $option['price_note'] ) ? (string) $option['price_note'] : cmbwc_get_default_service_price_note( $price_type, $is_deposit );
			$service_options[ $key ]['linked_product_id'] = ! empty( $option['linked_product_id'] ) ? absint( $option['linked_product_id'] ) : 0;
		}
...
/**
 * Get one service option by key.
 */
if ( ! function_exists( 'cmbwc_get_service_option_by_key' ) ) {
...
if ( ! function_exists( 'cmbwc_service_options_prepare_product_options_html' ) ) {
	function cmbwc_service_options_prepare_product_options_html( $product_options ) {
		$html = '';

		foreach ( $product_options as $product_id => $product_label ) {
			$html .= '<option value="' . esc_attr( $product_id ) . '">' . esc_html( $product_label ) . '</option>';
		}
 ...
 */
if ( ! function_exists( 'cmbwc_render_service_option_row' ) ) {
	function cmbwc_render_service_option_row( $index, $key = '', $option = array(), $product_options = array() ) {
		...
		<tr class="cmbwc-service-row">
			<td class="cmbwc-col-key" data-label="Nøgle">
				<input
					type="text"
			...
			<td class="cmbwc-col-label" data-label="Navn">
				<input
					type="text"
			...
			<td class="cmbwc-col-product" data-label="Koblet produkt">
				<select
			...
			<td class="cmbwc-col-price" data-label="Pris">
				<input
					type="number"
					step="0.01"
					min="0"
			...
			<td class="cmbwc-col-price-type" data-label="Prismodel">
				<select
			...
			<td class="cmbwc-col-deposit" data-label="Depositum">
				<select
			...
			<td class="cmbwc-col-note" data-label="Tekst under pris">
				<input
					type="text"
			...
			<td class="cmbwc-col-actions" data-label="Handling">
...
if ( ! function_exists( 'cmbwc_render_service_options_page' ) ) {
	function cmbwc_render_service_options_page() {
		...
		$product_options_html = cmbwc_service_options_prepare_product_options_html( $product_options );
		?>
		<div class="wrap cmbwc-admin-page cmbwc-admin-service-options-page">
			...
			<style>
				...
				.cmbwc-admin-service-options-page .cmbwc-service-help {
					max-width: 1440px;
					margin: 14px 0 18px;
					padding: 12px 14px;
			...
			<div class="cmbwc-service-help">
				<strong>Tip:</strong> Servicevalg bør normalt kobles til et WooCommerce-produkt, fordi kurv, kasse og ordrelinjer bruger det koblede produkt som selve service-/depositumslinjen.
			</div>
		...
		<script>
			document.addEventListener('DOMContentLoaded', function () {
				...
				const productOptionsHtml = <?php echo wp_json_encode( $product_options_html ); ?>;
