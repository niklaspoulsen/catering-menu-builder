...
function cmbwc_add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
	...
	$service_data = null;
	if ( $selected_service ) {
		$service_data = cmbwc_get_service_option_by_key( $selected_service );
	}
...
function cmbwc_display_cart_item_data( $item_data, $cart_item ) {
	...
	if ( ! empty( $data['included_names'] ) && is_array( $data['included_names'] ) ) {
		$item_data[] = array(
			'key'   => 'Indhold',
			'value' => implode( ', ', array_map( 'esc_html', $data['included_names'] ) ),
		);
	}
...
add_action( 'woocommerce_before_calculate_totals', 'cmbwc_before_calculate_totals', 20 );
...
function cmbwc_before_calculate_totals( $cart ) {
	...
	foreach ( $cart->get_cart() as $cart_item_key => $cart_item ) {
		...
		$line_total = ( $base_price * $covers ) + $addons_total + $service_total;

		/**
		 * We store whole menu configuration in one cart line.
		 * Therefore item quantity should remain 1 and line price should be the full total.
		 */
